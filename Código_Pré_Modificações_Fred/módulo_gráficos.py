...
def plotar_yield_curves(ws_solvente, yields_exp, yields_calc, informações_auxiliares):
    ...
    Inputs:
        ...
        informações_auxiliares (list) : lista contendo os elementos [DMA_formatado, tipo_cálculo_programa, nome_planilha] 
                                        a lista acima contém informações úteis para o nome do arquivo do gráfico a ser salvo na pasta 'Resultados'
    ...
    # Obs: se todos os yields experimentais são nulos (ex: 'nome_planilha' = 'Yanes_P3'), a curva experimental é plotada na cor branca (desaparece)
    if all(yield_exp == 0 for yield_exp in yields_exp): plt.plot(100*ws_solvente, 100*yields_exp, "o", mfc = "white", mec = "white", markersize = 10)
    if any(yield_exp != 0 for yield_exp in yields_exp): plt.plot(100*ws_solvente, 100*yields_exp, "o", mfc = "blue", mec = "black", markersize = 10)
    ...
    # Títulos dos eixos, valores mínimos e máximos de cada eixo, fontes das marcas de escala, marcas de escala secundárias
    ...
    if (tipo_cálculo_programa == 'predicao'): nome_arquivo_gráfico = f"{nome_planilha}_YIELDCURVE.png"
    if (tipo_cálculo_programa == 'regressao'): nome_arquivo_gráfico = f"{nome_planilha}_YIELDCURVE_tipo_regressao_{tipo_regressão}_algoritmo_otimizacao_{algoritmo_otimização}.png"
    
    # Salvando o gráfico
    diretório_da_pasta_deste_modulo = os.path.dirname(os.path.abspath(__file__))
    if (tipo_cálculo_programa == 'predicao'): diretório_do_png = os.path.join(diretório_da_pasta_deste_modulo, "Resultados", "Predição", nome_arquivo_gráfico)
    if (tipo_cálculo_programa == 'regressao'): diretório_do_png = os.path.join(diretório_da_pasta_deste_modulo, "Resultados", "Regressão", nome_arquivo_gráfico)
    plt.savefig(diretório_do_png, dpi = 300, bbox_inches = "tight")
...
def plotar_distribuição_massa_molar(MMsagregados, xsagregados, alfa, MWavg, informações_auxiliares):
    ...
    Inputs:
        ...
        alfa (float)                  : parâmetro de forma da função densidade de probabilidade da distribuição Gamma (FDP_Gamma)
    ...
    # Nome do arquivo do gráfico a ser salvo
    if (tipo_cálculo_programa == 'predicao'): nome_arquivo_gráfico = f"{nome_planilha}_DISTMASSAMOLAR.png"
    if (tipo_cálculo_programa == 'regressao'): nome_arquivo_gráfico = f"{nome_planilha}_DISTMASSAMOLAR_tipo_regressao_{tipo_regressão}_algoritmo_otimizacao_{algoritmo_otimização}.png"
