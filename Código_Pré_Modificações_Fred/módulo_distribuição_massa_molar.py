...
# Função
def gerar_distribuição_massa_molar(alfa, MWavg, n_agregados, MWmin, MWmax, tipo_cálculo_MM_agregados, método_integração_FDP_Gamma):
    ...
    Inputs:
        ...
        tipo_cálculo_MM_agregados (string)   : tipo de cálculo para a determinação das massas molares dos agregados de asfaltenos
    ...
    wsagregados = np.zeros(n_agregados)
...
if __name__ == "__main__":
    ...
    MMsagregados, wsagregados, xsagregados = gerar_distribuição_massa_molar(2.7822, 1859, 30, 700, 7200, "superior", "trapezios")

    # Leitura do arquivo recebido por e-mail
    diretório_deste_modulo = os.path.dirname(__file__)
    diretório_do_xlsx = os.path.join(diretório_deste_modulo, 'Outras referências', 'Sobre distribuição gama', 'distribuição_P1_Yanes.xlsx')
    ...
    df2 = pd.DataFrame({"MMs_Gaba": MMsagregados_Yanes, 
                        "MMs_Calc": MMsagregados, 
                        "MMs_DRA(%)": 100*np.abs(MMsagregados_Yanes - MMsagregados)/MMsagregados_Yanes,
                        "xs_Gaba": xssagregados_Yanes, 
                        "xs_Calc": xsagregados, 
                        "xs_DRA(%)": 100*np.abs(xssagregados_Yanes - xsagregados)/xssagregados_Yanes})
    
    # Exibição dos resultados
    print("\n+--------------------+--------------------+-----------------------+-------------+------------------------+----------------------+")
    print(f"| TESTE DA FUNCAO 'gerar_distribuicao_massa_molar'")
    print(tabulate(df2, headers = df2.columns, tablefmt = 'pretty', showindex = True))
# FIM DO TESTE
# ********************************************************************************************************************************************************************************* #