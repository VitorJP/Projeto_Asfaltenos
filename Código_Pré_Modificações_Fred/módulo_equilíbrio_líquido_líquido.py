...
def calcular_Delta_G_sistema(beta_a, xs_2a, xs_2b, xs_1, dados_modelo_termodinâmico):
    ...
    G_H = beta_a * calcular_tpd(xs_2a, xs_1, dados_modelo_termodinâmico)
    G_L = (1 - beta_a) * calcular_tpd(xs_2b, xs_1, dados_modelo_termodinâmico)
    Delta_G = 0.0 if np.abs(G_H + G_L) < 1e-8 else G_H + G_L
    return Delta_G
    ...
    def pares_binários(n):  # 3*N*(N-1)/2 partículas
        ...
        for i, j in combinations(range(n), 2):
            if zs[i] != 0 and zs[j] != 0:
                ...
                # (0.1, 0.9)
                composição = np.zeros(n)
                composição[i], composição[j] = 0.1, 0.9
                lista_composições.append(composição)
                # (0.9, 0.1)
                composição = np.zeros(n)
                composição[i], composição[j] = 0.9, 0.1
                lista_composições.append(composição)
        return lista_composições
...
def identificar_composição_com_G_mínimo(ws_candidatas, zs, dados_modelo_termodinâmico):
    ...
    i_relativo = np.argmin(casos_válidos)  # índice relativo dentro dos válidos
    i_absoluto = np.where(duas_fases)[0][i_relativo]  # índice absoluto no array completo

    return duas_fases[i_absoluto], beta[i_absoluto], xs_L[i_absoluto], xs_H[i_absoluto], n_it[i_absoluto]
