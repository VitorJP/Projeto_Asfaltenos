...
def calcular_composições_ELL(T, xs_completo, deltas, Vs, xsagregados):
    ...
    while erro > tol:
        ...
        Ks = np.exp(Vs/VmH - Vs/VmL + np.log(Vs/VmL) - np.log(Vs/VmH) + (Vs/(R*T))*(deltas - deltamL)**2 - (Vs/(R*T))*(deltas - deltamH)**2)
        ...
        try:
            ...
            if RachfordRice(limite_inferior_betarr)*RachfordRice(limite_superior_betarr) > 0: 
                raise ValueError(f"A funcao de Rachford-Rice nao muda de sinal com betarr entre [{limite_inferior_betarr}, {limite_superior_betarr}].")
            else:
            ...
            if(betarr < 0): print(f"Neste ponto, a funcao 'brentq' falhou e a 'fsolve' foi acionada para retornar betarr = {betarr:.4e}.")
